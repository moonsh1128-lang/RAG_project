...
## 가상 데이터 (`mock_data.py`)
실제 판결문/법령/심결례/유권해석 파일이 아직 없어서, 4개 Rag 소스마다 짧은 가상 샘플 문장 2~3개를 넣어뒀다. 전부 `[가상 샘플]`로 표시되어 있고 실제 법률 정보가 아니다. 실 데이터 파일이 준비되면 `mock_data.py`를 지우고 `rag_sources.py`의 `_store_for`를 실 데이터 로더로 교체하면 된다.
...
## 확정되지 않은 부분
- 실 데이터 파일이 준비되면 `mock_data.py` 기반 가상 검색을 실 데이터 검색으로 교체 필요
